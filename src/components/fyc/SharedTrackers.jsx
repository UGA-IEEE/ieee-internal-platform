...
export function SharedTrackers() {
  ...
  const isSharing = !!profile?.share_progress
  ...
  if (!isSharing) {
    return (
      <div className="rounded-xl border border-dashed border-gray-200 p-5 flex items-center gap-3 mb-8 text-gray-500">
        ...
        <p className="text-sm">
          Turn on sharing above to see classmates who are also sharing their tracker. Sharing is reciprocal — you can only view trackers from students who can see yours.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-gray-800">Classmates Sharing Their Tracker</h2>
      ...
      <div className="divide-y divide-gray-100">
        {peers.map(peer => (
          <button
            key={peer.id}
            onClick={() => navigate(`/fyc/member/${peer.id}`)}
