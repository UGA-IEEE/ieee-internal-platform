...
export function SharingToggle() {
  ...
  const shared = !!profile?.share_progress
  const pendingOffAt = profile?.share_off_requested_at ? new Date(profile.share_off_requested_at) : null
  const isPendingOff = shared && !!pendingOffAt

  async function updateSharing(nextValue) {
    setSaving(true)
    const { error } = await supabase.rpc('set_share_progress', { new_value: nextValue })
  ...
  function handleToggle() {
    if (shared && !isPendingOff) {
      ...
      if (!window.confirm(
        'Turning sharing on lets other FYC students view your applications right away. ' +
  ...
  return (
    <div className={`rounded-xl border p-4 flex items-center justify-between gap-4 ${shared ? 'bg-ieee-blue-light border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-gray-800">Share my tracker with other FYC members</p>
          <p className="text-xs text-gray-500 mt-0.5">
            {isPendingOff
              ? "Other FYC students can still view your applications until sharing turns off."
              : shared
                ? 'Other FYC students can view your applications (read-only).'
